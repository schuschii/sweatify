<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Exercises for ') }}<span id="body-part-name" class="font-semibold"></span>
        </h2>
    </x-slot>
    ...
    <script>
        ...
        const bodyPart = decodeURIComponent(window.location.pathname.split('/').pop()); // Extract body part from the URL

        // Set body part name in the header
        document.getElementById('body-part-name').textContent = bodyPart.charAt(0).toUpperCase() + bodyPart.slice(1);

        async function fetchExercises(resetOffset = false) {
            if (resetOffset) offset = 0; // Reset offset if limit changes

            limit = parseInt(document.getElementById('limit').value);
            const response = await fetch(`/api/exercises/body-part/${bodyPart}?limit=${limit}&offset=${offset}`, {
