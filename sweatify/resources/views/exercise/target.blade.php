<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Exercises for ') }}<span id="target-name" class="font-semibold"></span>
        </h2>
    </x-slot>
    ...
    <script>
        ...
        const target = decodeURIComponent(window.location.pathname.split('/').pop()); // Extract target name from the URL

        // Set target name in the header
        document.getElementById('target-name').textContent = target.charAt(0).toUpperCase() + target.slice(1);

        async function fetchExercises(resetOffset = false) {
            if (resetOffset) offset = 0; // Reset offset if limit changes

            limit = parseInt(document.getElementById('limit').value);
            const response = await fetch(`/api/exercises/target/${target}?limit=${limit}&offset=${offset}`, {
