...
# extract generated app key from .env
APP_KEY=$(grep APP_KEY .env | cut -d '=' -f2-)

# update .env.testing
if [ -f .env.testing ]; then
  echo "Syncing APP_KEY in .env.testing"
  perl -pi -e "s/^APP_KEY=.*/APP_KEY=${APP_KEY}/" .env.testing
else
  echo ".env.testing not found, skipping APP_KEY sync"
fi
